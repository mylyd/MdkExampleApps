...
public interface RetrofitUtils {
    ...
    @FormUrlEncoded
    @POST("recogliu.do?")
    Call<ResponseBody> CSpostAddInfo(@Field("img") String img, @Field("key") String key, @Field("secret") String secret, @Field("typeId") String typeid, @Field("format") String format);
    /**
     * 认证合一
     * @param img1 证件流
     * @param img2 实拍流
     ...
     */
    @POST("ocrInfo/add?")
    Call<ResponseBody> postAddCard(@Body RequestBody addCard);
}

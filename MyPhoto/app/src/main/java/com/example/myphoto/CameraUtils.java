...
class CameraUtils {
    ...
    private static int mCameraID = Camera.CameraInfo.CAMERA_FACING_BACK;//默认打开后置摄像头
    ...
    public static void openFrontalCamera(int expectFps) {
        ...
        for (int i = 0; i < numCameras; i++) {
            ...
            if (info.facing == Camera.CameraInfo.CAMERA_FACING_BACK) {//默认打开后置摄像头
        ...
        if (mCamera == null) {
            mCamera = Camera.open();
            mCameraID = Camera.CameraInfo.CAMERA_FACING_FRONT;//打开前置摄像头
        }
        // 没有摄像头时，抛出异常
        if (mCamera == null) {
